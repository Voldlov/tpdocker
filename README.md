...
2. Créer les conteneurs :
    - Frontend : `docker run -it --network ynov-frontend-network --name ynov-frontend --ip 10.0.0.10 -d -p 80:80 nginx` ;
    - Backend : `docker run -it --network ynov-backend-network --name ynov-backend --ip 10.0.1.10 -d -p 8080:80 -p 443:443 -e ALLOW_EMPTY_PASSWORD=yes bitnami/prestashop` ;
